...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    const namelist = new Array<any>();
    model.map((model: any) => {
      namelist.push({ IdModel: model?.IdModel, name: model?.name });
    });
...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      model.Gamme = CryptoJS.AES.encrypt(
        model.Gamme,
        password || ""
      ).toString();
      ...
      model
        .save()
        .then(() => res.status(200).json({ message: "Model edited!" }));
    } catch (error: any) {
      res.status(400).json({
