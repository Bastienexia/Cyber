...
router.post("/createRecette", async (req: Request, res: Response) => {
  ...
  RecetteCreate.puht = CryptoJS.AES.encrypt(RecetteCreate.puht, password).toString(); 
  RecetteCreate.gamme = CryptoJS.AES.encrypt(RecetteCreate.gamme, password).toString();
  ...
  if(req.body.description){
    RecetteCreate.description = CryptoJS.AES.encrypt(req.body.description, password).toString();
  }
  RecetteCreate.save()
    .then(() => {
...
router.put("/modify/:name", async (req: Request, res: Response) => {
  ...
  try{
    ...
    if(req.body.modelInfos.description){
      model.description = CryptoJS.AES.encrypt(req.body.description, password).toString();
    }
